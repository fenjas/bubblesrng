...
public class fileTools {
	...
	static String writeToBinFile(String workspacepath,
			List<Integer> RandomNumbers, int bytesToCapture, boolean shuffle, String options) throws IOException {
		...
		//Trap index error
		if (bytesToCapture > RandomNumbers.size()) {
			bytesToCapture = RandomNumbers.size();}
	...
	static void genStdRnd(String workspacepath, int samples) throws Exception {
		Random r = new Random();
		String rndBinFN = workspacepath + "java_std_prng_"
				+ dateFormat.format(d) + ".bin";
		DataOutputStream outBin = new DataOutputStream(new FileOutputStream(
				rndBinFN));

		try {
			for (int i = 0; i < samples; i++)
				outBin.writeByte(r.nextInt(256));
	...
	//Create a random number sample file using the crypto strong java prng class
	static void genSecRnd(String workspacepath, int samples) throws Exception {
	...
	// Copy ENT.exe under the current workspace folder
